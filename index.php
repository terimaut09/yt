...
<?php if(!empty($formats)){
  ...
  <div class="instruction_box">
    ...
    <h3>Unsupported.</h3>
    <p>This video is currently unsupported By Our System.<br>Try To Solve This Error Soon</p>
  </div>
  ...
  <?php }else{ ?>
         ...
         <div class="text_info">
            ...
            ?>
             ...
             <tr>
                 <td> <?php if($format->qualityLabel) echo $format->qualityLabel; else echo "Unknown"; ?>
               </td>
                 <td><?php if($format->mimeType) echo explode(";",explode("/",$format->mimeType)[1])[0]; else echo "Unknown";?>
               </td>
                 <td><a href="download_video.php?link=<?php echo urlencode($url)?>&title=<?php echo urlencode($title)?>&type=<?php if($format->mimeType) echo explode(";",explode("/",$format->mimeType)[1])[0]; else echo "mp4";?>" id="download_btn"><i class='bx bxs-download'></i>&nbsp;Download Now</a></td>
             </tr>
...
<?php } } }else{ ?>
  ...
  <center>
    <a href="../index.php">
  <button class="bubbly-button">Go Home</button>
</center>
</a>
